...
# NODE_RANK按需修改
#NODE_RANK=$1
#NODE_RANK=$(expr $(grep -n -o $HOSTNAME $HOSTFILE | awk -F: '{print $1}') - 1)
NODE_RANK=$(grep -n "$HOSTNAME" "$HOSTFILE" | cut -d: -f1 | awk '{print $1-1}')
echo $NODE_RANK
ulimit -c 0
...
EXPNAME=tp${TP_SIZE}_pp${PP_SIZE}_mbs${MICRO_BATCH_SIZE}_gbs${GLOBAL_BATCH_SIZE}_gpus${WORLD_SIZE}

mkdir -p $PROJ_HOME/output/$CURRENT_TIME
LOG_FILE=$PROJ_HOME/output/$CURRENT_TIME/$EXPNAME.log

CHECKPOINT_PATH=$PROJ_HOME/checkpoints_zj/$CURRENT_TIME/$EXPNAME
mkdir -p $CHECKPOINT_PATH

LOG_PATH=$PROJ_HOME/logs/$CURRENT_TIME/$EXPNAME
mkdir -p $LOG_PATH

TB_PATH=$PROJ_HOME/tboard/$CURRENT_TIME/$EXPNAME
mkdir -p $TB_PATH

WB_PATH=$PROJ_HOME/wandb/$CURRENT_TIME/$EXPNAME
mkdir -p $WB_PATH
...
# 13B 模型训练
#################################
cmd="PYTHONPATH="$ROOT_DIR/..":$PYTHONPATH torchrun $DISTRIBUTED_ARGS $ROOT_DIR/../../pretrain_llama.py \
    ${LLAMA_MODEL_ARGS[@]} \
    ${TRAINING_ARGS[@]} \
    ${MODEL_PARALLEL_ARGS[@]} \
    ${DATA_ARGS[@]} \
...
echo $cmd
eval $cmd 2>&1 | tee $LOG_FILE.$NODE_RANK
