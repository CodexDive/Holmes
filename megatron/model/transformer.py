...
def print_rank_last(message):
    """If distributed is initialized, print only on rank 0."""
    if torch.distributed.is_initialized():
        if torch.distributed.get_rank() == 47:
            print(message, flush=True)
    else:
        print(message, flush=True)
...
class ParallelTransformer(MegatronModule):
    ...
    def __init__(self, init_method, output_layer_init_method,
        ...
        # Number of layers.
        self.roce_ratio = None
        self.ib_ratio = None
        if args.use_asymmetric_pipeline_division:
            self.roce_ratio = args.roce_ratio / (args.roce_ratio + args.ib_ratio)
            self.ib_ratio = args.ib_ratio / (args.roce_ratio + args.ib_ratio)
            if args.use_ib_as_master:
                if mpu.is_pipeline_first_stage(ignore_virtual=True):
                    self.num_layers = int(args.num_layers * self.ib_ratio)
                else:
                    self.num_layers = int(args.num_layers * self.roce_ratio)
            else:
                if mpu.is_pipeline_first_stage(ignore_virtual=True):
                    self.num_layers = int(args.num_layers * self.roce_ratio)
                else:
                    self.num_layers = int(args.num_layers * self.ib_ratio)
            print_rank_0("HETNET-num_layers: {}".format(self.num_layers))
            print_rank_last("HETNET-num_layers: {}".format(self.num_layers))
        else:
            # Simple Mode: self.num_layers = args.num_layers // args.transformer_pipeline_model_parallel_size
        ...
        elif args.use_asymmetric_pipeline_division:
            if args.use_ib_as_master:
                if mpu.is_pipeline_first_stage(ignore_virtual=True):
                    offset = 0
                else:
                    offset = int(self.ib_ratio * self.num_layers)
            else:
                if mpu.is_pipeline_first_stage(ignore_virtual=True):
                    offset = 0
                else:
                    offset = int(self.roce_ratio * self.num_layers)
        else:
        ...
        print_rank_0("HETNET-offset: {}".format(offset))
        print_rank_last("HETNET-offset: {}".format(offset))
        ...
        if self.num_layers == 0:
            ...
            print_rank_0("HETNET-layers: {}".format([i + 1 + offset for i in range(self.num_layers)]))
            print_rank_last("HETNET-layers: {}".format([i + 1 + offset for i in range(self.num_layers)]))
