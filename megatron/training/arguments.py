...
def validate_args(args, defaults={}):
    ...
    # Check Asymmetric-Pipeline-Division args
    if args.use_asymmetric_pipeline_division:
        assert args.roce_ratio is not None and args.ib_ratio is not None, "The args values of roce_ratio and ib_ratio shouldn't be None in asymmetric_pipeline_division mode."
        assert args.num_layers % (args.roce_ratio + args.ib_ratio) == 0, "num_layers should be divisible by the sum of roce_ratio and ib_ratio."
        assert args.use_ib_as_master is not None or args.use_roce_as_master is not None
...
def _add_training_args(parser):
    ...
    group.add_argument('--use-hetnet',
                       action='store_true',
                       help='Enable HETNET in the training process')
    group.add_argument('--use-asymmetric-pipeline-division',
                       action='store_true',
                       help='Enable asymmetric-pipeline-division')
    group.add_argument('--use-ib-as-master',
                       action='store_true',
                       help='Whether use IB machine as master nodes')
    group.add_argument('--use-roce-as-master',
                       action='store_true',
                       help='Whether use RoCE machine as master nodes')
    group.add_argument('--roce-ratio', type=int, default=None,
                       help='Ratio of RoCE machines')
    group.add_argument('--ib-ratio', type=int, default=None,
                       help='Ratio of IB machines')
