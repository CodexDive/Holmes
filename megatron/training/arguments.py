...
def validate_args(args, defaults={}):
    ...
    if args.pipline_num_layers_list:
        if args.pipeline_model_parallel_size == 1:
            raise RuntimeError('--pipline-num-layers-list must use with pp > 1.')

    if args.pipline_num_layers_list:
        if args.pipeline_model_parallel_size != len(args.pipline_num_layers_list):
            raise RuntimeError('length of --pipline-num-layers-list must use same with pp.')

    if args.pipline_num_layers_list:
        if args.num_layers != sum(args.pipline_num_layers_list):
            raise RuntimeError('sum of --pipline-num-layers-list must use same with --num-layers.')
...
def _add_training_args(parser):
    ...
    group.add_argument('--pipline-num-layers-list', nargs='+', type=int, default=None,
                       help='a list of number of layers for each pipeline rank.')
    group.add_argument('--recompute-num-layers-list', nargs='+', type=int, default=None,
                       help='a list of recompute number of layers for each pipeline rank.')
    ...
    group.add_argument('--chatglm3-rotary-embed', action='store_true',
                       default=None,
                       help='use chatglm3 rotary embedding')
...
def _add_checkpointing_args(parser):
    ...
    group.add_argument('--train-from-scratch', action='store_true', default=None,
...
def _add_data_args(parser):
    ...
    group.add_argument('--padded-vocab-size', type=int, default=None,
                       help='padded-vocab-size for actural caculate')
