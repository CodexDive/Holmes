...
from megatron.core.models.common.embeddings.rotary_pos_embedding_fuse import RotaryEmbedding as RotaryEmbeddingFuse
from megatron.core.models.common.embeddings.rotary_pos_embedding_chatglm3 import  chatglm3_apply_rotary_pos_emb
...
class ParallelAttention(MegatronModule):
    ...
    def __init__(self, config, layer_number,
        ...
        self.use_flash_attn_rope = self.args.use_rotary_position_embeddings and self.args.rotary_percent == 1.0 \
                            and self.args.rotary_seq_len_interpolation_factor is None and self.args.use_rotary_emb_implement == 'flash_attn'
        ...
        if self.use_flash_attn:
            if flash_attn_unpadded_func is None or flash_attn_varlen_qkvpacked_func is None:
        ...
        self.rotary_emb_fuse = RotaryEmbeddingFuse(self.head_dim, device=get_current_device())

    def _checkpointed_attention_forward(self, query_layer, key_layer,
                                        value_layer, attention_mask,
                                        rotary_pos_emb=None):
        """Forward method with activation checkpointing."""
        rotary_pos_emb_ = Variable(torch.FloatTensor([1]).to(hidden_states.device)) if self.use_flash_attn_rope else rotary_pos_emb
        ...
        q_pos_emb, k_pos_emb = (None, None) if rotary_pos_emb_ is None \
            else rotary_pos_emb_
    ...
    def forward(self, hidden_states, attention_mask,
        ...
        if self.attention_type == AttnType.self_attn:
            ...
            indexes = torch.arange(mixed_x_layer.shape[0]*mixed_x_layer.shape[1])
        ...
        if rotary_pos_emb is not None and (not self.use_flash_attn_rope):
            if self.args.chatglm3_rotary_embed:
        ...
        #         dim = 2
        #     )
        # faster repeat_interleave from https://discuss.pytorch.org/t/repeat-interleave-without-cloning/136691/3 , only apply for dim=2 
        sk, b, ng, hn = key_layer.shape
        repeat = self.num_attention_heads_per_partition // self.num_query_groups_per_partition
        if repeat > 1:
            key_layer = key_layer.view(sk, b, ng, 1, hn).expand(sk, b, ng, repeat,hn).contiguous().view(sk, b, repeat * ng, hn)
            value_layer = value_layer.view(sk, b, ng, 1, hn).expand(sk, b, ng, repeat,hn).contiguous().view(sk, b, repeat * ng, hn)


        # apply relative positional encoding (rotary embedding)
        if rotary_pos_emb is not None:
            if self.args.chatglm3_rotary_embed:
                query_layer = chatglm3_apply_rotary_pos_emb(query_layer, rotary_pos_emb)
                key_layer = chatglm3_apply_rotary_pos_emb(key_layer, rotary_pos_emb)
            elif not self.use_flash_attn_rope:
                q_pos_emb, k_pos_emb = rotary_pos_emb
                ...
                qkv = self.rotary_emb_fuse(qkv, **kwargs)
                #qkv = rearrange(qkv, "(s b) three h d -> b s three h d", b = self.micro_batch_size)
        ...
        if not self.use_flash_attn:
            ...
            if not self.use_flash_attn_rope:
...
class ParallelTransformer(MegatronModule):
    ...
    def __init__(self, config,
        ...
        self.recompute_num_layers_list = args.recompute_num_layers_list
        ...
        self.use_flash_attn_rope = args.use_rotary_position_embeddings and args.rotary_percent == 1.0 \
                            and args.rotary_seq_len_interpolation_factor is None and args.use_rotary_emb_implement == 'flash_attn'
        ...
        if args.pipline_num_layers_list is not None:
            self.num_layers = args.pipline_num_layers_list[mpu.get_pipeline_model_parallel_rank()]
        else:
        ...
        elif args.pipline_num_layers_list is not None:
            offset = sum(args.pipline_num_layers_list[:mpu.get_pipeline_model_parallel_rank()])
            self.layers = torch.nn.ModuleList([build_layer(i + 1 + offset) for i in range(args.pipline_num_layers_list[mpu.get_pipeline_model_parallel_rank()])])
        else:
    ...
    def _checkpointed_forward(self, hidden_states, attention_mask,
        ...
        if self.transformer_impl == 'transformer_engine':
            ...
            if self.transformer_engine_v_0_10:
                te_forward_kwargs['rotary_pos_emb'] = rotary_pos_emb_
        ...
        if self.recompute_method == 'uniform':
            ...
            while l < self.num_layers:
                if self.transformer_impl == 'transformer_engine':
                    ...
                    hidden_states = tensor_parallel.checkpoint(
                        ...
                        encoder_output, enc_dec_attn_mask,
                        None, None, None, None, rotary_pos_emb_)
            ...
            if self.recompute_num_layers_list is not None:
                self.recompute_num_layers = self.recompute_num_layers_list[mpu.get_pipeline_model_parallel_rank()]
